...
def train(data_loader, net, criterion, optimizer,epoch):
    ...
    for i, (img, lbl) in enumerate(data_loader):
        ...
        if i % freq==0:
           print(f"\t[{i}/{len(data_loader)}], Loss {loss.item():.4f}  ,total_loss for freq {Loss1:.4f}")
           Loss1=0
...
def validation(data_loader, net, epoch):
    ...
    for img , lbl in data_loader:
        ...
        out_obj, ft = net(img, global_ft=True)
...
def save_checkpoint(val_state, res, net: nn.Module):
    ...
    torch.save(ckpt, str(save_dir / 'MVCNN.pth'))
    with open(str(save_dir / 'MVCNN.meta'), 'w') as fp:
        json.dump(res, fp)
